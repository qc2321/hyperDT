...
def embed(dists, space, burn_in_epochs=100, training_epochs=1000, lr=3e-4, print_freq=100):
    ...
    x_embed = np.concatenate(
        [component.embed_point(.1*np.random.rand(n, component.dim)) for component in space.components], axis=1
    )
    x_embed = np.nan_to_num(x_embed, 0)
    ...
    for epoch in range(burn_in_epochs + training_epochs):
        ...
        grad = np.random.rand(*x_embed.shape)
        x_embed = x_embed - lr * grad

        # Project back to manifold - TODO: do this better
        x_embed = np.concatenate(
            [c.embed_point(x[:, 1:]) for c, x in zip(space.components, space.split(x_embed))], axis=1
        )
