...
class RandomForestClassifier(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        return torch.mode(predictions, axis=0).values
    ...
    def predict_proba(self, X):
        ...
        return torch.mean(predictions, axis=0)
...
class HyperbolicRandomForestClassifier(RandomForestClassifier):
    def __init__(self, timelike_dim=0, curvature=-1, **kwargs):
        ...
        self.curvature = torch.abs(curvature)
        for tree in self.trees:
            tree.curvature = torch.abs(curvature)
...
class HyperbolicRandomForestRegressor(RandomForestClassifier):
    def __init__(self, timelike_dim=0, curvature=-1, **kwargs):
        ...
        self.curvature = ntorchp.abs(curvature)
        for tree in self.trees:
            tree.curvature = torch.abs(curvature)
        self.timelike_dim = self.tree_params["timelike_dim"] = timelike_dim
...
class ProductSpaceRF(BaseEstimator, ClassifierMixin):
    def __init__(
        self, signature, n_estimators=100, max_features="sqrt", max_samples=1.0, random_state=None, max_depth=3
    ):
        self.n_estimators = n_estimators
    ...
    def _generate_subsample(self, X, y):
        ...
        indices = torch.randint(n_samples, (sample_size,))
        return X[indices], y[indices]
