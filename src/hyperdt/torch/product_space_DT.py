...
class HyperspaceDecisionTree(HyperbolicDecisionTreeClassifier):
    ...
    def _get_candidates(self, X, dim):
        ...
        # Hypersphere case
            ...
            thetas = torch.atan2(X[:, self.timelike_dim], X[:, dim])
            thetas = torch.unique(thetas)  # sorted
            return (thetas[:-1] + thetas[1:]) / 2

        # Euclidean case
        else:
            unique_vals = torch.unique(X[:, dim])  # sorted
            return torch.atan2(torch.tensor([2], device=unique_vals.device), unique_vals[:-1] + unique_vals[1:])
    ...
    def _fit_node(self, X, y, depth):
        ...
        for dim in self.dims_ex_time:
            ...
            for theta in self._get_candidates(X=X[:, space_cols], dim=dim_in_space):
                ...
                min_len = min([len(y[left]), len(y[right])])
